...
const CoursePreview = ({
  ...
  const nextButton = () => {
    isEdit ? handleEditCourse() : handleCourseCreate();
  };
  return (
    <div className="w-[90%] m-auto py-5 mb-5">
      {courseData.courseData?.[0]?.videoUrl && (
        <CoursePlayer videoUrl={courseData.courseData[0].videoUrl} />
      )}
      ...
      <p className="text-[24px] text-blue-500 py-[16px]">
      ...
      <p>{courseData.courseData[0].description}</p>
      <div className="flex justify-between">
        <button
          className="bg-green-500 text-black px-[12px] rounded-2xl py-[4px]"
          onClick={() => prevButton()}
        >
          Prev
        </button>
